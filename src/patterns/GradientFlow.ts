...
type GradientFlowSerializedExtra = {
  curPolarity: GradientPolarity;
  curPosition: number;
  ringColors: RGB.Serialized[];
};

type GradientFlowProps = {
  color1: RGB;
  color2: RGB;
 ...
 */
export class GradientFlow extends BasePattern<GradientFlowProps> {
  ...
  private interpolation: Color[];

  constructor(props: GradientFlowProps) {
    super(props);
    this.interpolation = this.interpolateColors();
    this.ringColors = this.interpolation;
  }

  updateProps(props: GradientFlowProps) {
  ...
  serializeExtra(): GradientFlowSerializedExtra {
    return {
  ...
  deserializeExtra(object: GradientFlowSerializedExtra) {
