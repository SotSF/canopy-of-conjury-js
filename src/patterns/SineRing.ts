...
export class SineRing extends BasePattern {
  ...
  render(canopy: CanopyInterface) {
    ...
    while (angle <= Math.PI * 2) {
      ...
      const x = Math.floor(
        (radius + Math.sin(angle * this.values.frequency) * this.amp) *
          Math.cos(angle)
      );
      const y = Math.floor(
        (radius + Math.sin(angle * this.values.frequency) * this.amp) *
      ...
      let s = util.clampModular(
        co.strip + this.values.rotate * this.iteration,
        0,
        canopy.strips.length
      );
  ...
  serializeExtra(): SineRingSerializedExtra {
    return {
      amp: this.amp,
      dir: this.dir,
    };
  ...
  deserializeExtra(object: SineRingSerializedExtra) {
    this.amp = object.amp;
    this.dir = object.dir;
  }
}
