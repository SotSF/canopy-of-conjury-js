import { CanopyCoordinate, CanopyInterface } from "@/types";

type MemoMap = Record<string, CanopyCoordinate>;

export class MemoizedMap {
  map: MemoMap;

  constructor(canvasSize: number, canopy: CanopyInterface) {
    this.map = this.memoize(canvasSize, canopy);
  }
  ...
  private memoize(canvasSize: number, canopy: CanopyInterface): MemoMap {
    ...
    for (let x = 0; x <= canvasSize; x++) {
      for (let y = 0; y <= canvasSize; y++) {
        map[`${x}.${y}`] = mapToCanopy(x - halfCanvas, y - halfCanvas);
      }
    }
    ...
    function mapToCanopy(x: number, y: number) {
...
export class Memoizer {
  maps: Record<string, MemoizedMap> = {};
