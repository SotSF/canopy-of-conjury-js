...
type Bug = {
  center: Coordinate; // invisible center of bug's motion pattern
...
export class Swarm extends BasePattern {
  ...
  static defaultProps(): SwarmProps {
    return {
      color: RGB.random(),
    };
  }
    ...
    for (let i = 0; i < this.swarmSize; i++) {
      const center = {
        x:
          this.swarm.center.x +
          Math.floor(
            Math.random() * (this.maxSwarmRadius + this.maxSwarmRadius + 1) -
              this.maxSwarmRadius
          ),
        y:
          this.swarm.center.y +
  ...
  progress() {
    ...
    this.swarm.bugs.forEach((bug) => {
      ...
      if (
        Math.sqrt(
          (this.target.x - bug.center.x) ** 2 +
            (this.target.y - bug.center.y) ** 2
        ) > this.maxSwarmRadius
      ) {
        slowBugs++;
      }
    ...
    if (this.swarmClock === this.swarmTime) {
      ...
      const x =
        Math.floor(Math.random() * (this.dimension - 50)) - this.dimension / 2;
      const y =
        Math.floor(Math.random() * (this.dimension - 50)) - this.dimension / 2;
      this.target = { x, y };
    }
  ...
  render(canopy: CanopyInterface) {
    ...
    this.swarm.bugs.forEach((bug) => {
      const x =
        Math.floor(bug.radius * Math.cos(bug.theta)) + bug.center.x + half;
      const y =
        Math.floor(bug.radius * Math.sin(bug.theta)) + bug.center.y + half;
      const d = Math.sqrt(
        (this.target.x - bug.center.x) ** 2 +
          (this.target.y - bug.center.y) ** 2
      );
      if (_.inRange(x, 0, this.dimension) && _.inRange(y, 0, this.dimension)) {
        ...
        if (_.inRange(co.led, 0, canopy.stripLength)) {
          const alpha =
            d < this.maxSwarmRadius * 0.6
              ? (this.maxSwarmRadius - d) / this.maxSwarmRadius
              : 0.4;
          canopy.strips[co.strip].updateColor(
            co.led,
            this.values.color.withAlpha(alpha)
          );
        }
      }
    });
  ...
  serializeExtra(): SwarmSerializedExtra {
    return {
      swarm: this.swarm,
  ...
  deserializeExtra(obj: SwarmSerializedExtra) {
