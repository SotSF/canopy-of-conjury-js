...
type SerializedDrops = { drops: SerializedDrop[] };
type SerializedDrop = Omit<Drop, "color"> & {
...
export class Drops extends BasePattern {
  ...
  progress() {
    ...
    this.drops.forEach((drop) => {
      ...
      drop.opacity -= 0.01;
      if (drop.opacity <= 0) {
        this.drops = _.without(this.drops, drop);
      }
    });
  }
  ...
  serializeExtra(): SerializedDrops {
    return {
  ...
  deserializeExtra(obj: SerializedDrops) {
