...
type Star = CanopyCoordinate;
type ShootingStarsSerializedExtra = { stars: Star[] };
...
export class ShootingStars extends BasePattern {
  ...
  render(canopy: CanopyInterface) {
    this.stars.forEach((star) => {
      for (let l = 0; l <= this.values.trail; l++) {
        ...
        const color = this.values.color.withAlpha(
          this.values.opacity * ((this.values.trail - l) / this.values.trail)
        );
  ...
  serializeExtra(): ShootingStarsSerializedExtra {
    return {
      stars: this.stars,
    };
  }

  deserializeExtra(object: ShootingStarsSerializedExtra) {
