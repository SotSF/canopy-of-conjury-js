...
export class Venn extends BasePattern {
  ...
  render(canopy: CanopyInterface) {
    ...
    this.circles.forEach((circle) => {
      const x = Math.floor(
        centerRadius * Math.cos(TWO_PI / this.values.quantity) + half
      );
      const y = Math.floor(
        centerRadius * Math.sin(TWO_PI / this.values.quantity) + half
      );
      let t = 0;
      while (t < 100) {
        ...
        if (
          _.inRange(x2, 0, this.dimension) &&
          _.inRange(y2, 0, this.dimension)
        ) {
          ...
          if (_.inRange(co.led, 0, canopy.stripLength)) {
            for (let i = 0; i < this.values.quantity; i++) {
              const strip =
                co.strip +
                Math.floor((i * canopy.strips.length) / this.values.quantity);
              canopy.strips[strip % canopy.strips.length].updateColor(
                co.led,
                this.values.color.withAlpha((100 - circle) / 100)
              );
            }
          }
        }
  ...
  serializeExtra(): VennSerializedExtra {
    return {
      circles: this.circles,
    };
  }

  deserializeExtra(obj: VennSerializedExtra) {
