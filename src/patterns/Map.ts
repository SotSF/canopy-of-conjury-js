...
export class Map extends BasePattern {
  ...
  static getStripIndexForTime(time: number, canopy: CanopyInterface) {
    return Math.round(util.lerp(canopy.strips.length - 1, 0, time / 12));
  }

  static getStripIndexForLetter(l: number, canopy: CanopyInterface) {
  ...
  render(canopy: CanopyInterface) {
    ...
    for (
      let s = Map.getStripIndexForTime(10, canopy);
      s <= Map.getStripIndexForTime(2, canopy);
      s++
    ) {
    ...
    for (let l = 1; l < 13; l++) {
      for (
        ...
        canopy.strips[s].updateColor(
          Map.getStripIndexForLetter(l, canopy),
          this.colors.hour
        );
      }
      ...
      if (l == 6) {
        this.fGaps.forEach((gap) => {
          for (
            let i = Map.getStripIndexForTime(gap[1], canopy) + 1;
            i < Map.getStripIndexForTime(gap[0], canopy);
            i++
          ) {
    ...
    for (let h = 2; h <= 10; h += 0.5) {
      ...
      for (
        let l = esplanadeIndex;
        l <= Map.getStripIndexForLetter(12, canopy);
        l++
      ) {
    ...
    for (
      let s = Map.getStripIndexForTime(6.5, canopy) + 1;
      s < Map.getStripIndexForTime(5.5, canopy);
      s++
    ) {
